"""Tests for Text Normalization functionality in Python Readability."""
...
# Get all test cases for Text Normalization
TEXT_NORMALIZATION_TESTS = get_test_cases_by_functional_area(FunctionalArea.TEXT_NORMALIZATION)

# Create parameterized test cases with marks
test_cases_with_marks = [
    pytest.param(case_name, id=case_name)
    for case_name in TEXT_NORMALIZATION_TESTS
]


@pytest.mark.area_text_normalization
@pytest.mark.parametrize("case_name", test_cases_with_marks)
def test_text_normalization(case_name):
    """Run tests for text normalization.
